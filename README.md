...
> **第一次看这个仓库的同学，请先看下面这两个文件，其它都是支撑材料。**

## 先看这两个文件

| # | 文件 | 是什么 | 怎么打开 |
|---|------|--------|----------|
| 1 | [`订详FLipbook效果.html`](./订详FLipbook效果.html) | **核心成果 demo**：JourneyKit · 香港机场主动推送（订单详情 FLipbook 翻页效果） | 启动本地服务后访问 `http://127.0.0.1:8899/订详FLipbook效果.html` |
| 2 | [`正向支付链路.html`](./正向支付链路.html) | **正向支付链路入口页**：打开后会自动在新标签页跳转到 ①（保留当前页作为入口锚点） | 启动本地服务后访问 `http://127.0.0.1:8899/正向支付链路.html` |
...
> **注意**：必须通过 `http://127.0.0.1:8899/...` 访问，**不要双击 `file://` 打开** —— 因为这两个页面依赖 `<script type="module">` 加载 ES Module，浏览器在 `file://` 协议下会拒绝加载，会出现白屏。
