  ...
  return `
    <div class="composer${state?.composerActive ? " composer--active" : ""}${chips ? " composer--with-chips" : ""}">
      ...
      <div class="composer__bar">
        ${showSend || segPrefix ? "" : `<button class="composer__icon" aria-label="语音">${ICON.mic(20)}</button>`}
        ...
        ${showSend
          ...
          : `<button class="composer__icon" aria-label="相机">${ICON.camera(20)}</button>
             <button class="composer__icon" aria-label="更多">${ICON.plus(20)}</button>`}
      </div>
