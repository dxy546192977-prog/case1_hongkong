    ...
    <div class="ta-screen"${personaAttr} data-sheet-mode="${sheetMode}">
      ...
      <aside class="ta-sheet" id="ta-sheet" aria-label="中转管家详情">
        ...
        <div class="ta-sheet__body">
          ${renderBoardingInfoCard(t)}
          ${renderTransitTasks(t)}
        </div>
      </aside>

      ${renderComposer({ placeholder: "输入后查看前往路线…" })}
    </div>
  `;
}
...
function renderTransitSheetHead(d, t) {
  ...
  return `
    <div class="ta-sheet-head__top">
      <h2 class="ta-sheet-head__title">${d.headline} · 中转 ${t.layoverText}</h2>
...
// 航班信息卡：5 列等宽 grid（登机时间 / 航站楼 / 登机口 / 组别 / 座位号）
function renderBoardingInfoCard(t) {
  const b = t.boardingInfo;
  if (!b) return "";
  const cols = [
    { label: "登机时间", value: b.boardingTime },
    { label: "航站楼",   value: b.terminal },
    { label: "前往登机口", value: b.gate },
    { label: "组别",     value: b.group },
    { label: "座位号",   value: b.seat },
  ];
  return `
    <div class="ta-boarding-info">
      ${cols.map((c, i) => `
        <div class="ta-boarding-info__col${i < cols.length - 1 ? "" : " ta-boarding-info__col--last"}">
          <span class="ta-boarding-info__label">${c.label}</span>
          <b class="ta-boarding-info__value">${c.value}</b>
        </div>
      `).join("")}
    </div>
  `;
...
export function attachTransitStepMapSync(rootEl, { onStepSelect } = {}) {
  ...
  // 高亮当前选中的步骤（仅竖向列表，不联动横滑卡片选中态）
  function highlightStep(targetId) {
    rootEl.querySelectorAll(".route-leg.is-map-active").forEach((el) => {
...
function triggerHotspot(flipbook, targetId) {
  // 尝试找到对应 hotspot 并触发点击
  const hotspot = flipbook.querySelector(`[data-afp-hotspot][data-target="${targetId}"]`);
  if (hotspot) {
    hotspot.click();
    return;
  }
  // 没有对应 hotspot（如 overview 本身）—— 点 reset 回总览即可
  const resetBtn = flipbook.querySelector("[data-afp-reset]");
