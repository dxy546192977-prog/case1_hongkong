...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
export function buildDisruption(plan: any) {
  const legs: any[] = plan?.fullLegs || [];
  const flightIndex = legs.findIndex((l: any) => l.mode === "飞机");
  const pickupIndex = legs.findIndex((l: any) => l.mode === "接机");
  const busIndex = legs.findIndex((l: any) => /大巴|高铁|机场快线|机场巴士/.test(l.mode));
  const affectedIds = [busIndex, flightIndex, pickupIndex].filter((i: number) => i >= 0).map((i: number) => `s${i + 1}`);
  const flightLeg = legs[flightIndex] || legs.find((l: any) => l.mode === "飞机");
  const pickupLeg = legs[pickupIndex] || legs.find((l: any) => l.mode === "接机");
  ...
  return {
    ...
    impactList: legs.map((leg: any, index: number) => {
      const id = `s${index + 1}`;
