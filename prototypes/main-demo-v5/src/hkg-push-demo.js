...
function renderRouteCards(profile) {
  ...
  const nodeCards = profile.nodes
    .map((node, idx, all) => {
      ...
      const meta = [node.floor, node.estimatedTime].filter(Boolean).join(" · ");
      return `
...
function attachDetailSheet(rootFrame, profile) {
  ...
  // 地图 → 卡片高亮（is-selected 与原版 .route-leg.is-selected 一致）
  const sheetBody = sheet.querySelector(".itin-sheet__body");
  const highlight = (nodeId) => {
    rootFrame.querySelectorAll("[data-hkg-card]").forEach((row) => {
      row.classList.toggle("is-selected", row.dataset.hkgCard === nodeId);
    });
    if (!nodeId) return;
